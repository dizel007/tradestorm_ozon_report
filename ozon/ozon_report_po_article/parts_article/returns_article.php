...
foreach ($arr_type_items_WITH_POSTING_NUMBER['returns'] as $big_key => $items) {
        ...
        foreach ($our_item as $item) {
                ...
                $arr_article[$new_post_number]['post_info'][$new_post_number_full]['type']['RETURN'] = '_RETURN_'; // тип траты
...
// **************************************************************************
                ...
                if (($items['operation_type'] == 'OperationReturnGoodsFBSofRMS') || ($items['operation_type'] == 'OperationItemReturn')
                   ...
                   foreach ($items['services'] as $return_dop_obrabotka) 
                        {
                        $arr_article[$new_post_number]['post_info'][$new_post_number_full]['services'][$return_dop_obrabotka['name']] =
                        @$arr_article[$new_post_number]['post_info'][$new_post_number_full]['services'][$return_dop_obrabotka['name']]
        ...
        //************* Получение возврата, отмены, невыкупа от покупателя      */
                        ...
                        $arr_article[$new_post_number]['post_info'][$new_post_number_full]['amount'] = 
                        @$arr_article[$new_post_number]['post_info'][$new_post_number_full]['amount'] + $items['amount'];

                        $arr_article[$new_post_number]['post_info'][$new_post_number_full]['sale_commission'] = 
                        @$arr_article[$new_post_number]['post_info'][$new_post_number_full]['sale_commission'] + $items['sale_commission'];

                        $arr_article[$new_post_number]['post_info'][$new_post_number_full]['accruals_for_sale'] = 
                ...
                // количество возвратов в заказке
                        $arr_article[$new_post_number]['post_info'][$new_post_number_full]['count_return'] = 
                       @$arr_article[$new_post_number]['post_info'][$new_post_number_full]['count_return'] + 1;
                                } else {
